...
import { Tabs, Select, Flex, Text, Icon, Box, Portal, createListCollection } from '@chakra-ui/react';
...
const CodeOptions = ({ children }) => {
  ...
  const LanguageSelect = (
    ...
    >
      <Select.HiddenSelect />
      <Select.Control>
        <Select.Trigger fontSize="14px" h={8} w={16} bg="#060010" border="1px solid #392e4e" borderRadius="10px">
  ...
  return (
    <Tabs.Root mt={4} w="100%" variant="plain" defaultValue="default">
      <Tabs.List mb={4} w="100%" display="flex" alignItems="center" gap={2}>
        <Flex gap={2}>
      ...
      <Tabs.Content pt={0} value="default">
        {renderContent('css')}
      </Tabs.Content>
      <Tabs.Content pt={0} value="tailwind">
        {renderContent('tailwind')}
      </Tabs.Content>
    </Tabs.Root>
  );
};
